...
Solution MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < N - 1; i += 1) {
    ...
    sol.psi.push_back(solution.x[psi_start + i]);
    sol.v.push_back(solution.x[v_start + i]);
    sol.cte.push_back(solution.x[cte_start + i]);
    sol.epsi.push_back(solution.x[epsi_start + i]);
